        ...
        public override void OnInspectorGUI()
        {
            ...
            if (UI_PanelState.spriteCharacterTablePanel)
            {
                ...
                if (arraySize > 0)
                {
                    ...
                    if (tex != null)
                    {
                        Material mat = m_material_prop.objectReferenceValue aracterSerializedProperty(m_SpriteCharacterTableProperty.GetArrayElementAtIndex(elementIndex), ref spriteInfo_prop);
                ...
                m_scale = EditorGUI.FloatF                    {
                            if (string.IsNullOrEmpty(searchPattern) == false)
                            {
                                //GUIUtility.keyboardControl = 0;
                                m_CharacterSearchPattern = searchPattern.ToLower(System.Globalization.CultureInfo.InvariantCulture).Trim();

                                // Search Glyph Table for potential mategion Display Sprite Glyph Table
            EditorGUI.indentLevel = 0;
            rect = EditorGUILayout.GetControlRect(false, 24);
...
                                                                                                                                                                                                                                                                                                                                                                                                                                              m_SpriteCharacterTableProperty.arraySize;
